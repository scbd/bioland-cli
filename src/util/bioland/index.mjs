...
export async function initNewTestSite (country, { loadSeed, freshBackUp, upsertDNS } = { loadSeed: true, freshBackUp: true, upsertDNS: false }) {
  ...
  await syncNbsaps(country, { deleteOnly: false });
  await syncNationalReports(country, { deleteOnly: false });
  await syncNationalTargets(country, { deleteOnly: false });
}
...
export async function setTitleAndSlogan (countryCode) {
  ...
  for (const locale of locales) {
    ...
    if (exists) { await setConfigObject(countryCode, 'system.site', configObj, l); } else { await createConfigObject(countryCode, 'system.site', configObj, l); }
...
async function addDefaultCountry (countryCode, configKey, countryUuid) {
  ...
  const target_uuid = countryUuid; // eslint-disable-line camelcase
  const configObj = (await getConfigObject(countryCode, configKey));

  configObj.default_value = [{ target_uuid }]; // eslint-disable-line camelcase
  configObj.dependencies.content = [`taxonomy_term:countries:${target_uuid}`]; // eslint-disable-line camelcase
