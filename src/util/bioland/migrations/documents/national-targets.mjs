...
export default async function (site, { defaultCountry = 'fj', forceCountry, deleteOnly = false } = {}) {
  ...
  if (siteNationalTargets?.length) { await deleteDrupalDocuments(site, chmNationalTargets, siteNationalTargets); }

  if (!chmNationalTargets) return;

  if (deleteOnly) return;

  const chmDocIds = chmNationalTargets.map(({ _documentId_i }) => _documentId_i); // eslint-disable-line camelcase
...
async function getTemplate (doc) {
  ...
  const lang = doc?.header.languages[0];
  const title = doc?.title[lang];
  const uri = `https://chm.cbd.int/database/record?documentID=${documentId}`;
  const alias = `/implementation/targets/${slug(doc?.title[lang]).substring(0, 69)}`;
...
function getAichis (chmIdentifierObjects) {
  const identifiers = chmIdentifierObjects ? chmIdentifierObjects.map(({ identifier }) => identifier) : [];
