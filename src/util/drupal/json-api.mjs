...
export async function login (site) {
  try {
    ...
    const { csrf_token } = body; // eslint-disable-line camelcase

    global[site].token = csrf_token; // eslint-disable-line camelcase

    $http.set('X-CSRF-Token', csrf_token); // eslint-disable-line camelcase
...
export async function jsonApiDelete (site, { path, id, locale = '' }) {
  try {
    ...
    for (const identifier of identifiers) {
      const req = $http.delete(`${host}${localePrefix}/jsonapi/${path}/${identifier}`)
        .set('Content-Type', 'application/vnd.api+json');
