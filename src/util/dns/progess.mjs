...
async function isDnsUpdateDone (changes) {
  ...
  for (const { ChangeInfo } of changes) { checkPromises.push(route53.getChange({ Id: ChangeInfo.Id }).promise()); }

  const changesStatus = await Promise.all(checkPromises);

  for (const { ChangeInfo } of changesStatus) { if (ChangeInfo.Status === 'PENDING') return false; }
