...
export const getArgs = () => {
  for (const [index, value] of process.argv.entries()) {
    if (isValidCommand(value)) { return process.argv.slice(index + 1); }
  }
...
function getBranchFromPath () {
  for (const aBranchName of branches) {
    if (INIT_CWD && (INIT_CWD.endsWith(`/${aBranchName}`) || INIT_CWD.includes(`/${aBranchName}/`))) { return aBranchName; }
  }
...
function getBranchFromProps (alt = false) {
  ...
  for (const arg of process.argv) {
    if (isValidBranch(arg) && branchPath !== 'demo') { return arg; }
  }
