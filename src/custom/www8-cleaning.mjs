...
export default async (branch, commandArgs, { Util }) => {
    ...
    for (const countryCode of sites ){
        ...
        for (const { bundle, entity_id, field_image_url_uri } of rows )
            await  Util.dbSet(countryCode, `Update taxonomy_term__field_image_url SET  field_image_url_uri = ? WHERE entity_id = ${entity_id} AND bundle = '${bundle}' `, [(field_image_url_uri.replace('http://www8', 'https://www')).replace('http://', 'https://')])//'http://www8', 'https://www'
        ...
        for (const { bundle, entity_id, field_image_url_uri } of rows2 )
