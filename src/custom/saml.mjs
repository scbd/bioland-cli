...
export default async (branch, commandArgs, { Util }) => {
  ...
  for (const countryCode of sites) {
    ...
    const obj = await Util.getConfigObject(countryCode, 'core.menu.static_menu_link_overrides');

    await Util.dbSet(countryCode, 'UPDATE menu_tree SET enabled = ? WHERE menu_name LIKE \'%account%\' AND  id LIKE \'%samlauth.saml_controller_logout.logout%\'', [1]);
    await Util.dbSet(countryCode, 'UPDATE menu_tree SET enabled = ? WHERE menu_name LIKE \'%account%\' AND  id LIKE \'%user.logout%\'', [0]);

    if (obj.definitions.samlauth__saml_controller_logout__logout) { obj.definitions.samlauth__saml_controller_logout__logout.enabled = true; }

    if (obj.definitions.user__logout) { obj.definitions.user__logout.enabled = false; }

    await Util.setConfigObject(countryCode, 'core.menu.static_menu_link_overrides', obj);
...
function getEntityId (code, Util) {
  ...
  if (hasRedirectTo(code, Util) && !isDev && !isTest) { return `https://${hasRedirectTo(code)}`; } else {
    return isDev
